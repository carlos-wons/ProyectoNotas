 ...
 */
class fragment_ver_audios_tareas : Fragment() {
    ...
    private fun initialize(root: View, id: Int){
        ...
        val list: ArrayList<RecursosTarea>? = context?.let { daoRecursosTarea(it).getAllByAudioType(id) }
        if (list != null) {
            val adapter: adaptador_audio_tarea? = context?.let { adaptador_audio_tarea(it,list) }
